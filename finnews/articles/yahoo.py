...
from finnews.articles.utils import clean_html_text, string_contains, text_to_datetime, url_to_n3karticle
import asyncio
import json
import re
...
IGNORE_TEXT = [
    ...
    "Give feedback on",
    "Check out our" "WATCH: ",
    "SEE ALSO: " "has a disclosure policy",
    "More From ",
    "Image source: ",
    "©",
    "Benzinga.com",
...
class Yahoo(ArticleScraper):
    ...
    async def read_latest_headlines(self):
        ...
        for kterm in ["Edited Transcript of", "StreetEvents", "Bloomberg", "Zacks"]:
            term_resp = await self._get(
                "/v1/finance/search?q={}&lang=en-US&region=US&quotesCount=6&newsCount=4"
                ...
                + "&enableCb=true&enableNavLinks=true&enableEnhancedTrivialQuery=true".format(kterm),
                site="https://query1.finance.yahoo.com",
            )
            try:
