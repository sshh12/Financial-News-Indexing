...
class PRScraper:
    ...
    async def _get(self, url, method="GET", json_params=None, form_params=None, headers=HEADERS):
    ...
    async def read_prs_with_regex(
        self,
        regex,
        url_path,
        type_to_group={"date": 1, "url": 2, "title": 3},
        full_url_path=False,
        article_url_base=None,
        **kwargs
    ):
        ...
        for match in re.finditer(regex, resp):
            ...
            if len(title) == 0:
                continue
            releases.append((self.NAME.lower(), title, date, "", url))
...
class Gilead(PRScraper):
    ...
    async def read_prs(self):
        return await self.read_prs_with_regex(
            r'<p>\s*([\w, ]+)\s*<\/p>\s*<a href="([^"]+)">([^<]+)<\/a>', "/news-and-press/press-room/press-releases"
        )
...
class BioNTech(PRScraper):
    ...
    async def read_prs(self):
        return await self.read_prs_with_regex(
            r'field-nir-date">([^<]+?)<\/td>[\s\S]+?<a href="([^"]+?)" class="[\w\-]+">([^>]+?)<', "/press-releases"
        )
...
class Urogen(PRScraper):
    ...
    async def read_prs(self):
        return await self.read_prs_with_regex(
            r'date-time">([^<]+?)<\/div>[\s\S]+?<a href="([^"]+?)" hreflang="\w+">([^<]+?)<', "/news-releases"
        )
...
class SCWORX(PRScraper):
    ...
    async def read_prs(self):
        return await self.read_prs_with_regex(
            r'datetime="([\-\d :]+)">[\s\S]+?<a href="https:\/\/ir.scworx.com([^"]+)">([^<]+)<', "/press-releases",
        )
    ...
    async def read_prs(self):
        ...
        return await self.read_prs_with_regex(
            ...
            form_params={
                ...
                "data[page_id]": "3056",
                "data[style]": "pagination",
                "data[action]": "vc_get_vc_grid_data",
                "data[shortcode_id]": "1517419843788-afc1f200c7c29bd235e132b0777b3d80-2",
                ...
                "data[animation_in]": "",
                "data[animation_out]": "",
                "data[arrows_design]": "vc_arrow-icon-arrow_07_left",
                "data[arrows_color]": "chino",
                "data[arrows_position]": "outside",
                "data[paging_design]": "pagination_rounded",
                "data[paging_color]": "chino",
...
class Dynavax(PRScraper):
    ...
    async def read_prs(self):
        return await self.read_prs_with_regex(
            r'<p>([^<]+?)<\/p>[\s\S]+?<a href="([^"]+?)" hreflang="\w+">([^<]+?)<', "/press-releases"
        )
...
class VirBio(PRScraper):
    ...
    async def read_prs(self):
        return await self.read_prs_with_regex(
            r'date-time">([^<]+?)<[\s\S]+?<a href="([^"]+?)" hreflang="en">([^<]+?)<\/a>', "/press-releases"
        )
...
class Vaxart(PRScraper):
    ...
    async def read_prs(self):
        return await self.read_prs_with_regex(
            r'date-wrap">\s*([^<]+)<\/td>\s*?<td>\s*<a href="([^"]+?)" class="[\w\-]+">\s*?([^<]+?)<', "/press-releases"
        )
...
class Takeda(PRScraper):
    ...
    async def read_prs(self):
        return await self.read_prs_with_regex(
            r'ShortNumericalDate">\s*([/\d]+)\s*<[\s\S]+?<a href="([^"]+?)"[\s\S]+?<p>([^<]+?)<', "/investors/"
        )
...
class Catalyst(PRScraper):
    ...
    async def read_prs(self):
        return await self.read_prs_with_regex(
            r'date"><\/a>\s*(\w+ \d+, \d+)\s*<[\s\S]+?<a href="([^"]+?)" hreflang="en">([^<]+?)<', "/press-releases"
        )
...
class Biocept(PRScraper):
    ...
    async def read_prs(self):
        return await self.read_prs_with_regex(
            r'datetime">(\w+ \d+, \d+)<[\s\S]+?<a href="([^"]+?)" hreflang="en">([^<]+?)<', "/press-releases"
        )
...
class AbbVie(PRScraper):
    ...
    async def read_prs(self):
        return await self.read_prs_with_regex(
            r'date">(\w+ \d+, \d+)<[\s\S]+?<a href="([^"]+?)" target="_self" title="([^"]+?)"', "/news/press-releases/"
        )
...
class BioCryst(PRScraper):
    ...
    async def read_prs(self):
        return await self.read_prs_with_regex(
            r'<td>\s*(\w+ \d+, \d+)\s*?<[\s\S]+?<a href="([^"]+?)" hreflang="en">([^<]+?)<', "/press-releases"
        )
...
class Matinas(PRScraper):
    ...
    async def read_prs(self):
        return await self.read_prs_with_regex(
            r'datetime="([\d\- :]+)"[\s\S]+?<a href="([^"]+?)">([^<]+?)<', "/media/press-releases"
        )
...
class Exelixis(PRScraper):
    ...
    async def read_prs(self):
        return await self.read_prs_with_regex(
            r'>(\d+\/\d+\/\d+)<[\s\S]+?<a href="([^"]+?)" hreflang="en">([^<]+?)<', "/press-releases"
        )
