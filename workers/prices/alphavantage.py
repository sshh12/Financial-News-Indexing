...
class AlphaVantage(TickDataSource):
    ...
    async def read_stock_data(self, symbol, period=60, key_idx=0, attempt=0):
        ...
        if period == 60:
            url = "/query?function=TIME_SERIES_INTRADAY&symbol={}&interval=1min&apikey={}".format(
                symbol.strip(), api_key
            )
        else:
            url = "/query?function=TIME_SERIES_DAILY&symbol={}&outputsize=full&apikey={}".format(
                symbol.strip(), api_key
            )
    ...
    async def read_ticks(self, period=60):
        ...
        for symbols in _chunks(self.symbols, MAX_REQ_PER_MIN * len(self.api_keys)):
            ...
            fetch_tasks = [
                self.read_stock_data(symbol.upper(), key_idx=i % len(self.api_keys), period=period)
