...
class SEC(MetaDataSource):
    ...
    async def read_latest_filings(self):
        resp = await self._get('/cgi-bin/browse-edgar?action=getcurrent&CIK=&type=&company=&dateb=&owner=include&start=0&count=40&output=atom')
...
class SECFileParser:
    ...
    def parse_text(self):
        ...
        delete_tags = [
            'XBRL'
            'div', 'font', 'FONT', 'p', 'body',
            'P', 'U', 'B', 'DIV', 'HTML',
            'TEXT', 'I', 'span', 'SPAN', 'a',
            'ix:nonNumeric', 'html',
            'ix:nonFraction', 'ix:continuation'
        ]
        repl_regex = [
            ('<\/?hr[^>]*?>', '\n\n'),
            ('<\/?br[^>]*?>', '\n\n'),
