...
SIGNALS = set([
    'Top Gainers', 'New High', 'Over Bought', 'Unusual Volume', 
    'Upgrades', 'Earnings Before', 'Insider Buying', 'Top Losers', 'New Low', 
    'Most Volatile', 'Most Active', 'Downgrades', 'Earnings After', 'Insider Selling'
])
IMPACTS = {
    '1': 'LOW',
    '2': 'MEDIUM',
...
class FinViz(MetaDataSource):
    ...
    async def read_stats(self):
        ...
        for event in events:
            stats.append(dict(source='finviz', type='calendar',
                event_time=event[0], name=event[1], impact=IMPACTS.get(event[2]),
                event_for=event[3], actual=event[4], expected=event[5], prev=event[6]
            ))
        return 'finviz', stats