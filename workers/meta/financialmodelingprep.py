...
class FinancialModelingPrep(MetaDataSource):
    ...
    async def read_list(self, path):
        ...
        for item_json in resp[name]:
            items.append(dict(
                ...
                delta_percent=float(item_json['changesPercentage']\
    ...
    async def read_lists(self):
        lists = []
        fetch_tasks = [self.read_list(path) for path in 
            ['stock/actives', 'stock/gainers', 'stock/losers']
        ]
    ...
    async def read_group_stats(self):
        ...
        for item_json in sectors_resp['sectorPerformance']:
            items.update({item_json['sector']: {
                'delta_percent': float(item_json['changesPercentage']\
                    .replace('%', '').replace(')', '').replace('(', ''))
            }})
        groups = [
            GroupStats('sectors', 'fmp', items)
        ]
        return groups